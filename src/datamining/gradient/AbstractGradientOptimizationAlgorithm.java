...
{	
	...
	public void apply(int iterations)
	{
		...
		for(int t=0; t<iterations; t++)
		{
			...
			lastPara = this.getParameter();
			tmpPara = this.objectiveFunction.gradient(this.data, lastPara);
			...
			if(this.ascOrDesc) this.parameterVS.mul(tmpPara, this.learningFactor);
			else this.parameterVS.mul(tmpPara, -this.learningFactor);
						
			// adds the last parameter to the scaled gradient.
			this.parameterVS.add(tmpPara, lastPara);
			
			// store the new parameter
			this.updateParameter(tmpPara);			
				
			// iteration complete
			this.iterationComplete();			
			if(this.parameterMetric.distanceSq(lastPara, tmpPara) < this.epsilon*this.epsilon) break;
		}
	}
